...
function generateActionItemsResponse(meetings, person, allNames) {
  ...
  return { textResponse: formatTextForTTS(textResponse), meetings };
}
...
function formatTextForTTS(text) {
  return text.replace(/\b([A-Z]{2,})\./g, '$1') // Remove periods from acronyms
             .replace(/(\d{1,2}:\d{2})\./g, '$1') // Remove periods from time references
             .replace(/(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?)\s/g, ' ') // Ensure periods only end sentences
             .replace(/\.\s/g, ' ') // Remove periods followed by space
...
function formatDateTime() {
  ...
  const now = new Date();
  const prTime = now.toLocaleString('en-US', options);
  const [date, time] = prTime.split(', ').slice(1); // Split and remove weekday

  return {
    date,
    time,
    full: `It is ${time} Atlantic Standard Time on ${date}`,
    cardData: {
      type: 'dateTime',
      content: {
        date,
...
exports.local_llm = functions.https.onRequest(async (req, res) => {
...
- Get meeting details based on any person, date, or meeting title.
- Retrieve a list of all meetings in the database.

Only use the provided tools when the user specifically asks about meetings, action items, or people in the company directory. For all other queries, respond based on your general knowledge without calling any functions.

**Guidelines:**

- Provide information directly and professionally.
- If you're going to list items, do so in a conversational manner. For example, you could say: "First there is [fact #1.]" once you're done with that list item, say something like, "Then you have [fact #2]" then you could say, "Lastly, you have [fact #3]." This makes the information easier to digest and makes the conversation more natural.
...
- For questions unrelated to meetings or the company, respond using your general knowledge without calling any functions.
...
`;
  ...
  try {
    ...
    // Using improved context for the prompt
    let enhancedPrompt = prompt;
    if (lastQuery) {
      enhancedPrompt = `Considering our previous discussion about "${lastQuery}", here is my new question: "${prompt}"`;
    } else {
    ...
    // Update time patterns to be more inclusive
    const timePatterns = [
      /what (?:time|date) is it/i,
      /what(?:'s| is) today(?:'s| s)? date/i,
      /what day is (?:it|today)/i,
      /tell me the (?:time|date)/i,
      /show (?:me )?(?:the )?(?:current )?(?:time|date)/i,
      /(?:what|show me) today's date/i,
      /(?:what|what's) the date/i,
      /(?:current|today's) date/i,
      /(?:give|tell) me (?:the )?(?:current )?(?:time|date)/i
    ];

    // Check if the prompt matches any time patterns
    if (timePatterns.some(pattern => pattern.test(enhancedPrompt))) {
      console.log('Time/Date pattern matched:', enhancedPrompt);
      const dateTimeInfo = formatDateTime();
      res.status(200).json({
        response: {
          text: dateTimeInfo.full,
          cardData: dateTimeInfo.cardData
        }
      });
      return;
    }
    ...
    console.error('Error in local_llm:', error);
